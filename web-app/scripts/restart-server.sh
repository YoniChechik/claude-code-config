...
# Use build-and-start-prod.sh (it will handle killing the old one)
./scripts/build-and-start-prod.sh 2>&1 | grep -v "connection refused" | grep -v "channel.*open failed"
