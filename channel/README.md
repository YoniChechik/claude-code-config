...
- `GET http://127.0.0.1:<port>/health` — health check, returns `ok`
...
```bash
# Claude calls get_port → gets e.g. 8790
# Then launches the CI watcher with that port:
bash ci_watch_persistent.sh 8790 my-feature-branch
