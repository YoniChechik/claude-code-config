...
# _sanitize_shell_text and _extract_subshell_spans (used below) now live in
# the shared library sourced above. Only the git-specific "does a subshell
# span contain a git write" check stays local, since it needs the git-specific
# GIT_WRITE_SUBCMD pattern set up in the Bash branch below.
...
if [ "$tool_name" = "Bash" ]; then
    ...
    # heredoc body are inert prose and must not trip the rule. _sanitize_shell_text() keeps
    ...
    sanitized_command=$(_sanitize_shell_text "$command")
    ...
    while IFS= read -r segment; do
        ...
        #
        # `normalized_segment` (shared-library, _shell_command_guard.sh) has any
        # `command`/`env`/backslash/`VAR=` prefix already peeled off, so
        # `command bash -c "..."` and friends still hit these checks.
        normalized_segment="$(_strip_leading_wrappers "$segment")"
        if echo "$normalized_segment" | grep -qE "$BYPASS_SHELL_LEAD" && echo "$normalized_segment" | grep -qE "[[:space:]]${GIT_WRITE_SUBCMD}([[:space:]]|\$|\"|')"; then
            has_bypass_shell=1
            has_git_write=1
            break
        fi
        if echo "$normalized_segment" | grep -qE "$EVAL_LEAD" && echo "$normalized_segment" | grep -qE "[[:space:]]${GIT_WRITE_SUBCMD}([[:space:]]|\$|\"|')"; then
            has_eval=1
            has_git_write=1
            break
        fi
        # Check if this segment is a git write operation (covers env-prefix, absolute-path,
        # `-C`, and git-global-flag variants: `-c`, `--no-pager`, `--git-dir=`, `--work-tree=`,
        # all stripped by _strip_git_global_flags before the pattern is applied).
        normalized_git_segment="$(_strip_git_global_flags "$normalized_segment")"
        if echo "$normalized_git_segment" | grep -qE "$GIT_WRITE_PATTERN"; then
            has_git_write=1
            ...
            git_c_target=$(echo "$normalized_git_segment" | grep -oE "^([A-Z_][A-Z0-9_]*=[^[:space:]]*[[:space:]]+)*(/[^[:space:]]+/)?git[[:space:]]+-C[[:space:]]+[^[:space:]]+" | head -n1 | awk '{
