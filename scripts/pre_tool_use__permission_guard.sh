...
# ---------------------------------------------------------------------------
# gh — PR merges. Deliberately NOT a text-pattern check: a raw
# `gh api repos/<owner>/<repo>/pulls/<n>/merge -X PUT ...` call looks
# byte-for-byte identical whether the PR's required checks are green or
# failing — that's exactly how DEV incident 2752 slipped an admin-bypass
# squash-merge past this hook with zero prompt (this form is also the one
# used all session, every time, in place of `gh pr merge`, because `gh pr
# merge` itself is known to hang in this environment).
#
# So instead of guessing from the command string, ask GitHub for the PR's
# live merge state before deciding:
#   - mergeStateStatus == CLEAN  -> required checks are genuinely green, no
#     bypass involved -> proceed silently, no prompt (regular merge).
#   - anything else (BLOCKED, DIRTY, UNSTABLE, the query failing, gh/jq
#     erroring, etc.) -> treat as a possible admin/bypass merge -> ask.
# Fails closed: if we can't positively confirm CLEAN, we ask.
# ---------------------------------------------------------------------------
gh_merge_owner_repo_num() {
    # Sets globals MERGE_OWNER_REPO / MERGE_PR_NUM (either may be empty,
    # meaning "let gh infer it from the current branch/repo").
    local seg="$1"
    MERGE_OWNER_REPO=""
    MERGE_PR_NUM=""

    # `gh api repos/<owner>/<repo>/pulls/<num>/merge` (also matches the full
    # https://api.github.com/repos/... form).
    if [[ "$seg" =~ repos/([A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+)/pulls/([0-9]+)/merge(\?|$|[[:space:]]) ]]; then
        MERGE_OWNER_REPO="${BASH_REMATCH[1]}"
        MERGE_PR_NUM="${BASH_REMATCH[2]}"
        return 0
    fi

    # `gh pr merge [<num>|<url>] ...` — the number/url is optional (defaults
    # to the current branch's PR).
    if [[ "$seg" =~ ^gh[[:space:]]+pr[[:space:]]+merge([[:space:]]|$) ]]; then
        if [[ "$seg" =~ pr[[:space:]]+merge[[:space:]]+([0-9]+) ]]; then
            MERGE_PR_NUM="${BASH_REMATCH[1]}"
        elif [[ "$seg" =~ github\.com/([A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+)/pull/([0-9]+) ]]; then
            MERGE_OWNER_REPO="${BASH_REMATCH[1]}"
            MERGE_PR_NUM="${BASH_REMATCH[2]}"
        fi
        return 0
...
gh_merge_state() {
    # Echoes "clean" only when GitHub itself reports the PR's merge state as
    # CLEAN (no failing/pending required check, no bypass). Echoes
    # "not-clean" for every other outcome, including query failure.
    local owner_repo="$1" num="$2"
    local -a args=(pr view)
    [ -n "$num" ] && args+=("$num")
    [ -n "$owner_repo" ] && args+=(-R "$owner_repo")
    args+=(--json mergeStateStatus)

    local out state
    out=$(gh "${args[@]}" 2>/dev/null) || { echo "not-clean"; return; }
    [ -n "$out" ] || { echo "not-clean"; return; }
    state=$(echo "$out" | jq -r '.mergeStateStatus // empty' 2>/dev/null)
    [ "$state" = "CLEAN" ] && echo "clean" || echo "not-clean"
}

for segment in "${SEGMENTS[@]}"; do
    gh_merge_owner_repo_num "$segment" || continue
    if [ "$(gh_merge_state "$MERGE_OWNER_REPO" "$MERGE_PR_NUM")" != "clean" ]; then
        ask "PR merge is not reporting a clean merge state (a required check is failing/pending, the PR is blocked, or merge state could not be verified) — this looks like an admin/bypass merge. Confirm before proceeding; do not retry without explicit approval."
    fi
    # else: GitHub reports CLEAN — required checks are genuinely green and
    # no bypass is involved. Proceed silently, no prompt.
done
